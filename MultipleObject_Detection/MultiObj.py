...
import numpy as np
#Load YOLO Algorithm
net=cv2.dnn.readNet("yolov3.weights","configuredect.cfg")
...
for i in range(len(boxes)):
    if i in indexes:
        ...
        cv2.rectangle(img, (x, y), (x + w, y + h), (169,10,137), 2)
        cv2.putText(img, f' {classes[class_ids[i]].upper()} {int(confidences[i]*100)}% ' , (x, y-10), cv2.FONT_HERSHEY_PLAIN, 1, (0, 0, 204), 2)
